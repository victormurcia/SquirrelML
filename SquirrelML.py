...
def main():
    ...
    with st.expander("Location and Time"):
        st.write('Enter coordinates below. Click/Tap the map above to aid you.')
        col1, col2, col3 = st.columns(3)
        with col1:
            X = st.number_input('Longitude', format="%.5f", min_value=-73.981159, max_value=-73.949722,step=0.0001)
        with col2:
            Y = st.number_input('Latitude', format="%.5f", min_value=40.764911, max_value=40.800119,step=0.0001)
